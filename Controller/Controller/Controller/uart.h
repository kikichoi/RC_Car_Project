...
void uart0_transmit_string_with_length(const char *str, int length)
{
	for (int i = 0; i < length; i++)
	{
		uart0_transmit(str[i]); // 문자열 전송
	}
}
...
void uart1_transmit_string_with_length(const char *str, int length)
{
	for (int i = 0; i < length; i++)
	{
		uart1_transmit(str[i]); // 문자열 전송
	}
}
